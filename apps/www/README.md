...
Test: `pnpm --filter @r301/www test` runs `test/site.test.mjs` on Node's built-in runner — still no dependencies. It pins what ships in `public/` (an explicit allowlist), the head tags, every local asset path, the manifest, the zero-radius rule, the colour tokens' contrast in both schemes, and the reduced-motion gate. The root `pnpm test` runs it alongside the API suite.
