...
Paste **`prompts/04-telemetry.md`** into a fresh session.
Verify first (prompt 03 done-criteria): from repo root `pnpm test` (44 green) and `pnpm typecheck`; then `pnpm --filter @r301/api dev` and `curl -si http://127.0.0.1:8787/v1/nope | head -12` → 404 JSON envelope + `X-Request-Id`.
The HTTP foundation is in place: `src/index.ts` splits surfaces by hostname, both surfaces stamp `X-Request-Id`, and every `/v1` error renders the canonical envelope via `middleware/errors.ts`. **Routes raise `ApiError`** — no route should build an envelope by hand. Telemetry hooks into the same request id (design.md §8/§9).
