...
Paste **`prompts/05-health-and-smoke-v1.md`** into a fresh session.
Verify first (prompt 04 done-criteria): from repo root `pnpm test` (68 green) and `pnpm typecheck`; then `grep -r "D23 pinned" apps/api/src apps/api/test* -l | wc -l` → 4 (≥2 required).
Telemetry is live on both surfaces: one allowlist log line per request and Sentry wired via `withSentry`. **`GIT_SHA` is deliberately unwired** — prompt 05 owns it, and `sentryOptions()` in `src/telemetry/sentry.ts` is where `release` belongs. Nothing may be added to the log line or the Sentry event without checking the two `*.pinned.test.ts` files first (CLAUDE.md hard rule).
