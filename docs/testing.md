...
- `vitest.config.ts` (created by the first implementation prompt): workers pool pointed at `wrangler.toml`, **isolated storage per test** (each test gets fresh D1/KV), and a setup step that applies every file in `migrations/` in order to the test D1 — so the migration chain is proven on every run (PRD §14: "N−1 code on N schema" starts with "migrations apply from zero").
- Integration style: build the Hono app and call `app.request(...)` (or the pool's `SELF.fetch`) — full middleware chain, no HTTP server needed.
